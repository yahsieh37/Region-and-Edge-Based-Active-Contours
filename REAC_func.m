function seg = REAC_func(img_file,seg_file, max_its, thresh, a, b, R_bias)
%% Parameters
% max_its: max iterations of curve evolution
% thresh: stopping criterion (change of curve) of curve evolution
% a: parameter for region-based regularization
% b: parameter for edge-based regularization
% R_bias: bias between region- and edge-based level sets
...
%         end
        ...
        dphidt = R_bias*normGrad.*(F./max(abs(F))) + (1-R_bias)*(dphidt1 + dphidt2) + b*normGrad.*curvature + a*normGrad;
